namespace Client.Game.Context.Agents;

using Client.Accounting;
using Client.Game.Context.Data;
public class Agent : IEntity
{
    private Agent m_Parent;
    public int Serial { get; }
    public short X { get; private set; }
    public short Y { get; private set; }
    public sbyte Z { get; private set; }
    public Agent Parent
    {
        get => m_Parent;
        private set
        {
            m_Parent = value;
            OnParentChanged();
        }
    }
    public Dictionary<int, Agent> Children { get; } = new();
    public Agent(int serial) => Serial = serial;
    public Agent? WorldRoot
    {
        get
        {
            for (Agent e = this; e.Parent != null; e = e.Parent)
            {
                if (e is Agent && e.InWorld)
                    return e as Agent;
            }
            return null;
        }
    }
    public bool InWorld => m_Parent is WorldAgent;
    protected virtual void Attached(Agent child) { }
    protected virtual void Detached(Agent child) { }
    protected virtual void OnChangedLocation() { }
    protected virtual void OnParentChanged() { }
    protected virtual void OnDelete() { }
    private void Attach(Agent child)
    {
        if (child.Parent != this)
        {
            if (child.Parent != null)
                child.Parent.Detach(child);

            if (child is Item ||
                child is MobileAgent)
            {
                Children[child.Serial] = child;
            }

            Attached(child);
            child.Parent = this;
        }
    }
    private void Detach(Agent child)
    {
        if (child.Parent == this)
        {
            if (Children.ContainsKey(child.Serial))
                Children.Remove(child.Serial);

            Detach(child);
            child.Parent = null;
        }
    }
    public int DistanceTo(int x, int y)
    {
        x = X - x;
        y = Y - y;

        return (int)Math.Sqrt(x * x + y * y);
    }
    public bool IsChildOf(Agent agent)
    {
        if (agent != null)
        {
            for (Agent i = m_Parent; i != null; i = i.Parent)
                if (i == agent)
                    return true;
        }
        return false;
    }
    public IAccount? Account { get; set; } = null;
    public bool IsDeleted { get; set; } = false;
    public void Delete()
    {
        OnDelete();

        List<Agent> entities = new List<Agent>(Children.Values);

        while (entities.Count > 0)
            entities[entities.Count - 1].Delete();

        Children.Clear();
        IsDeleted = true;
    }
    public void SetLocation(short x, short y, sbyte z)
    {
        X = x;
        Y = y;
        Z = z;

        OnChangedLocation();
    }
    public void SetParent(Agent parent)
    {
        if (Parent != parent)
        {
            if (Parent != null)
                Parent.Detach(this);

            if (parent != null)
                parent.Attach(this);
        }
    }
}